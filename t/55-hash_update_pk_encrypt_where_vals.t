...
$db->hash_update(href=>\%update, table => 'dbix_raw', where => "name=?", vals => ['Adam'], pk => {name => 'id', val => 1}, encrypt => '*');

my $person = $db->raw(query => "SELECT name, age, favorite_color FROM dbix_raw where id=1", decrypt => '*');
